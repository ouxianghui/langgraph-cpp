...
- request `timeout_` / `deadline_` 到期返回 `DeadlineExceeded`。cpp-httplib 常把响应等待
  超时改写成 `Error::Read`（Darwin 上 SO_RCVTIMEO 也常见此码）；带 deadline 的请求会把
  `Read`/`Write`/`Timeout`/`ConnectionTimeout` 收敛为 `DeadlineExceeded`（payload 超限除外）。

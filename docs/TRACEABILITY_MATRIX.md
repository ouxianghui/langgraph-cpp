...
| FR-007 | model/tool loop 不依赖真实 provider。 | [ARCHITECTURE.md](ARCHITECTURE.md) | `src/langgraph/message`、`src/langgraph/model`、`src/langgraph/tool` | `agent_loop_test.cpp`、`provider_chat_model_test.cpp` | `model_tool_model_loop` | 默认 examples；usage metadata normalization |

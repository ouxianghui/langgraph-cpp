...
{
    auto update = lgc::StateUpdate::fromJson(std::move(text));
    assert(update.isOk());
...
[[nodiscard]] lgc::CompiledStateGraph buildCountingGraph()
{
    ...
    assert(graph.addConditionalEdges(
        ...
        [](const lgc::State& state, lgc::Runtime&) -> lgc::Result<lgc::NodeId> {
            return state.view().at("count").get<int>() < 3 ? lgc::NodeId("tick") : lgc::NodeId(lgc::END);
        },
...
{
    auto base = lgc::RunnableConfig::fromJson({
    ...
    auto call = lgc::RunnableConfig::fromJson({
    ...
    auto merged = lgc::mergeRunnableConfigs({ *base, *call });
    assert(merged.isOk());
    ...
    auto patched = lgc::patchRunnableConfig(*merged, {
    ...
    auto applied = lgc::applyRunnableConfig(lgc::RunOptions {}, *merged);
    assert(applied.isOk());
...
{
    lgc::StateGraph graph;
    assert(graph.addNode("approve", [](const lgc::State&, lgc::Runtime& context) -> lgc::Result<lgc::StateUpdate> {
    ...
    assert(graph.addEdge("approve", std::string(lgc::END)).isOk());
    auto compiled = graph.compile();
    assert(compiled.isOk());
    ...
    assert(replayedPause->status_ == lgc::RunStatus::Paused);
...
void testInterruptNodeRerunAndReplayAfterCompletion()
{
    ...
    firstResume.command_ = lgc::Command::resume({ { "approval", "yes" } });
    ...
    assert(replayed->status_ == lgc::RunStatus::Paused);
    ...
    secondResume.command_ = lgc::Command::resume({ { "approval", "again" } });
    auto replayCompleted = compiled->resume("compat-interrupt-rerun", secondResume);
    assert(replayCompleted.isOk());
    assert(replayCompleted->status_ == lgc::RunStatus::Completed);
...
void testSequentialInterruptReplayAndResumeErrors()
{
    ...
    lgc::StateGraph multi;
    auto approvalNode = [](std::string id) {
        return [id = std::move(id)](const lgc::State&, lgc::Runtime& context) -> lgc::Result<lgc::NodeOutput> {
            ...
            auto update = lgc::StateUpdate::fromJsonValue({ { id, context.resumeValue() } });
            if (!update.isOk())
    ...
    assert(multi.addEdge(std::string(lgc::START), "left").isOk());
    assert(multi.addEdge(std::string(lgc::START), "right").isOk());
    assert(multi.addEdge("left", std::string(lgc::END)).isOk());
    assert(multi.addEdge("right", std::string(lgc::END)).isOk());
    auto compiledMulti = multi.compile();
    assert(compiledMulti.isOk());

    lgc::RunOptions multiOptions;
    multiOptions.threadId_ = "compat-multi-interrupt-error";
    multiOptions.checkpointer_ = std::make_shared<lgc::InMemorySaver>();
    ...
    wrongIdOptions.checkpointer_ = std::make_shared<lgc::InMemorySaver>();
    auto wrongIdPaused = compiledMulti->invoke(stateFromJson("{}"), wrongIdOptions);
    assert(wrongIdPaused.isOk());
    assert(wrongIdPaused->status_ == lgc::RunStatus::Paused);
...
void testSendAndCommandBoundaries()
{
    ...
    assert(sendGraph.addNode("fan", [](const lgc::State&, lgc::Runtime&) -> lgc::Result<lgc::StateUpdate> {
    ...
    assert(sendGraph.addEdge(std::string(lgc::START), "fan").isOk());
    ...
    lgc::RunOptions sendOptions;
    sendOptions.reducers_.set("items", lgc::ReducerKind::Append);
    ...
    lgc::StateGraph commandGraph;
    assert(commandGraph.addNode("decide", [](const lgc::State&, lgc::Runtime&) -> lgc::Result<lgc::NodeOutput> {
        return lgc::NodeOutput::command(lgc::Command::gotoNode(
            "finish",
            updateFromJson(R"({"routed":true})")));
    }).isOk());
    assert(commandGraph.addNode("finish", [](const lgc::State& state, lgc::Runtime&) -> lgc::Result<lgc::StateUpdate> {
        assert(state.view().at("routed") == true);
        return lgc::StateUpdate::fromJson(R"({"finished":true})");
    }).isOk());
    assert(commandGraph.addEdge(std::string(lgc::START), "decide").isOk());
    assert(commandGraph.addCommandRoute("decide", { "finish" }).isOk());
    assert(commandGraph.addEdge("finish", std::string(lgc::END)).isOk());
...
void testSubgraphNamespaceAndStreamEnvelope()
{
    ...
    assert(child.addNode("child_node", [](const lgc::State&, lgc::Runtime& context) -> lgc::Result<lgc::StateUpdate> {
        assert(context.streamWriter().write("child-event", { { "inside", true } }).isOk());
        return lgc::StateUpdate::fromJson(R"({"child":true})");
    }).isOk());
    assert(child.addEdge(std::string(lgc::START), "child_node").isOk());
    assert(child.addEdge("child_node", std::string(lgc::END)).isOk());
    ...
    assert(parent.addSubgraph("child", childPtr, lgc::SubgraphOptions {
        .persistence_ = lgc::SubgraphPersistenceMode::PerInvocation,
        .checkpointNamespace_ = "child",
    }).isOk());
    assert(parent.addEdge(std::string(lgc::START), "child").isOk());
    assert(parent.addEdge("child", std::string(lgc::END)).isOk());
...
void testTokenStreamEnvelope()
{
    ...
    lgc::StateGraph graph;
    assert(graph.addNode("model", lgc::makeModelNode(model, lgc::ModelNodeOptions {
        .stream_ = true,
    })).isOk());
...
void testStreamProjectionGoldenDetails()
{
    ...
    for (;;) {
        ...
        if ((*part)->mode_ == lgc::StreamMode::Updates) {
            sawUpdate = true;
        ...
        if ((*part)->mode_ == lgc::StreamMode::Messages) {
        ...
        if ((*part)->mode_ == lgc::StreamMode::Checkpoints) {
            sawCheckpoint = true;
        ...
        if ((*part)->mode_ == lgc::StreamMode::Debug) {
            sawDebug = true;
...
{
    lgc::StateGraph graph;
    assert(graph.addNode("tick", [](const lgc::State& state, lgc::Runtime&) {
        const auto count = state.view().value("count", 0);
        return lgc::StateUpdate::fromJsonValue({ { "count", count + 1 } });
    }).isOk());
    assert(graph.addEdge(std::string(lgc::START), "tick").isOk());
    assert(graph.addEdge("tick", std::string(lgc::END)).isOk());
    auto compiled = graph.compile();
    assert(compiled.isOk());
    ...
    for (;;) {
        ...
        if (!part->has_value())
            break;
        const auto& data = (*part)->data_;
        assert(data.at("type") == ((*part)->mode_ == lgc::StreamMode::Updates ? "updates" : "values"));
    ...
    assert(interruptGraph.addNode("pause", [](const lgc::State&, lgc::Runtime& context) -> lgc::Result<lgc::StateUpdate> {
        auto answer = context.interrupt("approval", { { "question", "continue?" } });
        if (!answer.isOk())
            return answer.status();
        return lgc::StateUpdate::fromJsonValue({ { "approved", *answer } });
    }).isOk());
    assert(interruptGraph.addEdge(std::string(lgc::START), "pause").isOk());
    assert(interruptGraph.addEdge("pause", std::string(lgc::END)).isOk());
...
void testInterruptAndErrorStreamGoldenDetails()
{
    ...
    for (;;) {
        ...
        if (!part->has_value())
            break;
        if ((*part)->mode_ == lgc::StreamMode::Interrupts) {
    ...
    assert(errorGraph.addNode("boom", [](const lgc::State&, lgc::Runtime&) -> lgc::Result<lgc::StateUpdate> {
        return lgc::Status::failedPrecondition("boom");
    }).isOk());
    assert(errorGraph.addEdge(std::string(lgc::START), "boom").isOk());
    assert(errorGraph.addEdge("boom", std::string(lgc::END)).isOk());
    ...
    for (;;) {
        ...
        if (!part->has_value())
            break;
        if ((*part)->mode_ == lgc::StreamMode::Events && (*part)->data_.at("event") == "on_node_error") {
            sawNodeErrorEnvelope = true;
            assert((*part)->data_.at("data").at("error").at("message").get<std::string>().find("boom") != std::string::npos);
        }
        if ((*part)->mode_ == lgc::StreamMode::Events && (*part)->data_.at("event") == "on_chain_error") {
...
{
    lgc::StateGraph leaf;
    assert(leaf.addNode("leaf_node", [](const lgc::State&, lgc::Runtime& context) -> lgc::Result<lgc::StateUpdate> {
    ...
    assert(leaf.addEdge(std::string(lgc::START), "leaf_node").isOk());
    assert(leaf.addEdge("leaf_node", std::string(lgc::END)).isOk());
    auto compiledLeaf = leaf.compile();
    assert(compiledLeaf.isOk());

    lgc::StateGraph middle;
    assert(middle.addSubgraph("leaf", std::make_shared<lgc::CompiledStateGraph>(*compiledLeaf), lgc::SubgraphOptions {
        .persistence_ = lgc::SubgraphPersistenceMode::PerThread,
        .checkpointNamespace_ = "leaf",
    }).isOk());
    assert(middle.addEdge(std::string(lgc::START), "leaf").isOk());
    assert(middle.addEdge("leaf", std::string(lgc::END)).isOk());
    auto compiledMiddle = middle.compile();
    assert(compiledMiddle.isOk());

    lgc::StateGraph parent;
    assert(parent.addSubgraph("mid", std::make_shared<lgc::CompiledStateGraph>(*compiledMiddle), lgc::SubgraphOptions {
        .persistence_ = lgc::SubgraphPersistenceMode::PerThread,
        .checkpointNamespace_ = "mid",
    }).isOk());
    assert(parent.addEdge(std::string(lgc::START), "mid").isOk());
    assert(parent.addEdge("mid", std::string(lgc::END)).isOk());
    auto compiledParent = parent.compile();
    assert(compiledParent.isOk());
    ...
    auto streamResult = compiledParent->streamProjected(
        ...
        options,
        lgc::RunProjectionOptions {
            .modes_ = { lgc::StreamMode::Events, lgc::StreamMode::Custom },
            .capacity_ = 64,
            .includeSubgraphs_ = true,
            .langGraphProtocol_ = true,
        });
    ...
    for (;;) {
        ...
        if (!part->has_value())
            break;
        if ((*part)->mode_ == lgc::StreamMode::Custom && (*part)->name_ == "leaf-event") {
            sawLeafCustom = true;
    ...
    auto middleHistory = checkpointer->list(lgc::CheckpointListOptions {
    ...
    auto leafHistory = checkpointer->list(lgc::CheckpointListOptions {
    ...
    auto parentOnlyHistory = checkpointer->list(lgc::CheckpointListOptions {
    ...
    auto childRootHistory = checkpointer->list(lgc::CheckpointListOptions {
        .threadId_ = "compat-nested/mid/leaf",
    ...
    lgc::StateGraph commandChild;
    assert(commandChild.addNode("child_node", [](const lgc::State&, lgc::Runtime&) -> lgc::Result<lgc::NodeOutput> {
        return lgc::NodeOutput::command(lgc::Command::gotoParentNode(
            "after",
            updateFromJson(R"({"from_child":true})")));
    }).isOk());
    assert(commandChild.addEdge(std::string(lgc::START), "child_node").isOk());
    auto compiledCommandChild = commandChild.compile();
    assert(compiledCommandChild.isOk());

    lgc::StateGraph commandParent;
    assert(commandParent.addSubgraph("child", std::make_shared<lgc::CompiledStateGraph>(*compiledCommandChild)).isOk());
    assert(commandParent.addNode("after", [](const lgc::State& state, lgc::Runtime&) -> lgc::Result<lgc::StateUpdate> {
        assert(state.view().at("from_child") == true);
        return lgc::StateUpdate::fromJson(R"({"after":true})");
    }).isOk());
    assert(commandParent.addEdge(std::string(lgc::START), "child").isOk());
    assert(commandParent.addCommandRoute("child", { "after" }).isOk());
    assert(commandParent.addEdge("after", std::string(lgc::END)).isOk());
...
void testNestedSubgraphInterruptResumeAndHistory()
{
    ...
    assert(leaf.addEdge(std::string(lgc::START), "leaf_pause").isOk());
    assert(leaf.addEdge("leaf_pause", std::string(lgc::END)).isOk());
    auto compiledLeaf = leaf.compile();
    assert(compiledLeaf.isOk());
    ...
    auto middleHistory = checkpointer->list(lgc::CheckpointListOptions {
        ...
        .checkpointNamespace_ = std::string("root|mid"),
        .order_ = lgc::CheckpointListOrder::NewestFirst,
    });
    assert(middleHistory.isOk());
    ...
    lgc::RunOptions resumeOptions = options;
    resumeOptions.command_ = lgc::Command::resume({ { "leaf-approval", "ok" } });
    ...
    auto completedLeafHistory = checkpointer->list(lgc::CheckpointListOptions {
        .threadId_ = "compat-nested-interrupt/mid/leaf",
        .checkpointNamespace_ = std::string("root|mid|leaf"),
        .order_ = lgc::CheckpointListOrder::NewestFirst,
    });
...
{
    auto routeRegistry = std::make_shared<lgc::ToolRegistry>();
    auto routeRegistered = routeRegistry->add(std::make_shared<lgc::FunctionTool>(
        lgc::ToolSpec {
            .name_ = "route",
    ...
    lgc::StateGraph commandGraph;
    assert(commandGraph.addNode("tools", lgc::ToolNode(routeRegistry)).isOk());
    assert(commandGraph.addNode("finish", [](const lgc::State& state, lgc::Runtime&) -> lgc::Result<lgc::StateUpdate> {
        assert(state.view().at("tool_routed") == true);
        assert(state.view().at("messages").size() == 2);
        return lgc::StateUpdate::fromJson(R"({"finished":true})");
    }).isOk());
    assert(commandGraph.addEdge(std::string(lgc::START), "tools").isOk());
    ...
    lgc::RunOptions commandOptions;
    commandOptions.reducers_.set("messages", lgc::ReducerKind::AddMessages);
    ...
    auto interruptRegistry = std::make_shared<lgc::ToolRegistry>();
    auto calls = std::make_shared<std::atomic<int>>(0);
    auto interruptRegistered = interruptRegistry->add(std::make_shared<lgc::FunctionTool>(
        lgc::ToolSpec {
            .name_ = "approve",
            .description_ = "interrupt before tool side effect",
        },
        [calls](const lgc::ToolRequest& request, lgc::ToolRuntime& context) -> lgc::Result<lgc::ToolResult> {
            ...
            return lgc::ToolResult::success({ { "approved", *approved } });
        }));
    assert(interruptRegistered.isOk());

    lgc::StateGraph interruptGraph;
    assert(interruptGraph.addNode("tools", lgc::ToolNode(interruptRegistry)).isOk());
    assert(interruptGraph.addEdge(std::string(lgc::START), "tools").isOk());
    assert(interruptGraph.addEdge("tools", std::string(lgc::END)).isOk());
    auto compiledInterrupt = interruptGraph.compile();
    assert(compiledInterrupt.isOk());
    ...
    interruptOptions.reducers_.set("messages", lgc::ReducerKind::AddMessages);
    ...
    lgc::RunOptions resumeOptions = interruptOptions;
    resumeOptions.command_ = lgc::Command::resume({ { "tool-approval", true } });
    auto completed = compiledInterrupt->resume("compat-tool-interrupt", resumeOptions);
    assert(completed.isOk());
    assert(completed->status_ == lgc::RunStatus::Completed);
    assert(calls->load(std::memory_order_relaxed) == 2);
    auto messages = lgc::messagesFromStateJson(completed->state_.view());
    assert(messages.isOk());
    assert(messages->size() == 2);
    assert(messages->back().type_ == lgc::MessageType::Tool);
